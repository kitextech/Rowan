...
public struct Line {
    ...
    public func rotated(_ quaternion: Quaternion) -> Line {
        return Line(start: start.rotated(quaternion), end: end.rotated(quaternion))
    }
...
public let e_x = Vector(x: 1, y: 0, z: 0)
public let e_y = Vector(x: 0, y: 1, z: 0)
public let e_z = Vector(x: 0, y: 0, z: 1)
...
//    }
    ...
    public func collapsed(on bases: (x: Vector, y: Vector)) -> NSPoint {
        return NSPoint(x: component(along: bases.x), y: component(along: bases.y))
    }

    public func collapsed(along axis: Vector) -> NSPoint {
        if axis || e_z {
            return NSPoint(x: y, y: x)
        }
